...
ex) EXTERN sub1@0:PROC // 다른 모듈인 sub1@0 프로시저를 여기에 호출(@n - 매개변수에 따라 바이트가 정해짐), @0은 매개변수가 아예없음, 만약 @4이면 매개변수가 1개 있음

<b>EXTERNDEF</b>
EXTERNDEF 지시어는 PUBLIC과 EXTERN을 동시에 대체할 수 있는 지시어입니다.
ex) .data
    EXTERNDEF myVar:DWORD // myVar이라는 변수를 다른 모듈에서도 접근 및 참조 가능하게 해줌
...
- <mark>캡슐화</mark> -
<b>PRIVATE</b> -
Private지시어는 프로시저나 심볼을 현재 모듈 내부에서만 사용할 수 있도록 제한하는 한정자입니다.
여러 모듈에서 같은 이름의 프로시저가 있을 때 발생할 수 있는 이름 충돌을 피할 수 있습니다.
만약 아무런 설정을 하지 않으면 변수와 심볼은 기본적으로 자신이 속한 모듈 안에서만 접근할 수 있는 private 상태입니다.
ex) mysub Proc Private // mysub라는 프로시저는 이 모듈 내부에서만 쓸 수 있습니다.
